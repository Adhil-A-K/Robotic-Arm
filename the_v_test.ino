...
void runSequence() {
  ...
  // Wrist-last transition helper targets to avoid pressing object downward.
  int transitNoWrist[NUM_SERVOS] = {
    transitAngles[CH_BASE],
    transitAngles[CH_SHOULDER],
    transitAngles[CH_ELBOW],
    currentAngles[CH_WRIST],
    transitAngles[CH_GRIPPER]
  };

  int dropNoWrist[NUM_SERVOS] = {
    presets[dropIdx].angles[CH_BASE],
    presets[dropIdx].angles[CH_SHOULDER],
    presets[dropIdx].angles[CH_ELBOW],
    currentAngles[CH_WRIST],
    presets[dropIdx].angles[CH_GRIPPER]
  };
  ...
  switch (seqStep) {
    ...
    case SEQ_TRANSIT_WITH_OBJECT:
      Serial.printf("[SEQ] Step 4 — Transit with object (wrist moves last)\n");
      // Move base/shoulder/elbow first, keep current wrist until they settle.
      setAllAngles(transitNoWrist);
      setServoAngle(CH_GRIPPER, GRIPPER_CLOSE);  // keep closed during transit
      // Then move wrist to transit target as last priority.
      setServoAngle(CH_WRIST, transitAngles[CH_WRIST]);
      seqStep++;
      break;

    case SEQ_GOTO_DROP:
      Serial.printf("[SEQ] Step 5 — Move to drop%d (wrist moves last)\n", dropIdx - 1);
      // Move base/shoulder/elbow to drop first, keep wrist until final.
      setAllAngles(dropNoWrist);
      setServoAngle(CH_GRIPPER, GRIPPER_CLOSE);  // keep closed
      // Finally align wrist to drop pose.
      setServoAngle(CH_WRIST, presets[dropIdx].angles[CH_WRIST]);
      seqStep++;
      break;
